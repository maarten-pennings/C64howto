...
Een blinky laat een LEDje knipperen.

_Blinky1541_ wordt een 6502 machine taal programma. 
De C64 "upload" het naar de 1541. Als de 1541 het uitvooert
...
Daaruit leren we dat `control port B` (om pinnen laag of hoog te trekken)
op adres $1C00 is gemapped. PB3 is het derde bitje op dat adres.
...
plaatsen we ons programma?

In dezelfde memory map zien we dat de 1541 vijf buffers heeft,
op pagina 3 (0300-03FF) tot en met pagina 4 (0700-07FF). Wij 
gaan de eerste buffer gebruiken. Zolang we geen file `LOAD` of `SAVE` 
doen heeft de 1541 firmware die buffers niet nodig.


## Het blinky programma


Zoals een blinky betaamd, schrijven we een kort programma.
Omdat we alleen bit 3 willen veranderen doen we een zogeheten 
_read-modify-write_; we lezen $1C00, maskeren bit 3 naar nul en schrijven 
het resultaat terug naar $1C00: `LDA $1C00; AND #$F7; STA $1C00`.
We doen hetzelfde om de bit weer naar een te schrijven. Omdat het
allemaal te snel gaat roepen we een subroutine aan die executie vetraagt
(een "wait").
...
- De wavhtroutine bevindt zich op $0320. 
  ...
  De wachttijd van twee geneste 256-loops is ingeveer 1/3 seconde.
...
twee bytes, _little endian_ (dwz _least significant byte_ eerst)
De schrijf en lees opdracht heeft daarna nog een argument: een byte 
...
- De regels 1x openen het disk drive commando kaneel (kanaal 15 of apparaat 8).
...
- De regels 3x doen meerdere memory writes "M-W" naar de disk van `D$` 
  (een command mag maximaal 40 bytes bevatten).
